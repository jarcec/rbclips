...
\section{Constraits}
CLIPS provide strong mechanism for defining contraits (limitation) of slots in Templates and Classes. In rbClips it's surrounded by class Constrait. But keep on your mind that this is only prescription that is not saved in CLIPS environment. This means that if you use Constrait object in creating some slot, this slot will use this constraits that are set in time of creation, but if you after change  the constrait, it will not change the slot.
...
There is also an option to use block to set up constrait object that accept one variable (Clips::Constrait:Creator) that have these methods:
\begin{itemize}
...
Examples:
\begin{verbatim}
 c1 = Clips::Constrait.new :name => 'animal'
\end{verbatim}

\textbf{Possible problems:} Constrait object don't check how is set up. All checking is done when using constrait object (for example when creating slot). So some problems are reported when using not when creating. For example CLIPS prohibit to set values and range at the same time and so on...
